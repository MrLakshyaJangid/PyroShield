...
const RegisterSupplier = () => {
  ...
  const fetchSuppliers = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_URL}/api/suppliers`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(`${import.meta.env.VITE_URL}/api/suppliers/${editingId}`, formData);
        setMessage("✅ Supplier updated successfully!");
      } else {
        await axios.post(`${import.meta.env.VITE_URL}/api/suppliers`, formData);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this supplier?')) {
      try {
        await axios.delete(`${import.meta.env.VITE_URL}/api/suppliers/${id}`);
