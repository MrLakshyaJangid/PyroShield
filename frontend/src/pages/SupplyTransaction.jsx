...
const SupplyTransaction = () => {
  ...
  const fetchSuppliers = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_URL}/api/suppliers?sortByTotalItems=true&order=${sortOrder}`);
  ...
  const fetchItems = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_URL}/api/items`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        ...
        await axios.put(`${import.meta.env.VITE_URL}/api/suppliers/${supplierId}/items/${itemId}`, {
          Price: formData.Price
        });
        setMessage("✅ Supply transaction updated successfully!");
      } else {
        await axios.post(`${import.meta.env.VITE_URL}/api/suppliers/${formData.Supplier_ID}/items`, {
  ...
  const handleDelete = async (supplierId, itemId) => {
    if (window.confirm('Are you sure you want to delete this supply transaction?')) {
      try {
        await axios.delete(`${import.meta.env.VITE_URL}/api/suppliers/${supplierId}/items/${itemId}`);
